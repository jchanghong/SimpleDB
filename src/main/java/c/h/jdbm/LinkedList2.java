...
class LinkedList2<E> extends AbstractSequentialList<E> {
    ...
    private final class Iter implements ListIterator<E> {
        ...
        public void remove() {
            ...
            try {
                if (lastOper == 1) {
                    //last interfacs was next() so remove previous element
                    lastOper = 0;
                    ...
                    //last interfacs was prev() so remove next element
                    lastOper = 0;
        ...
        public void set(E value) {
            ...
            try {
                if (lastOper == 1) {
                    //last interfacs was next(), so update previous item
                    lastOper = 0;
                    Entry<E> n = db.fetch(prev, entrySerializer);
                    n.value = value;
                    db.update(prev, n, entrySerializer);
                } else if (lastOper == -1) {
                    //last interfacs was prev() so update next item
                    lastOper = 0;
                    Entry<E> n = db.fetch(next, entrySerializer);
                    n.value = value;
